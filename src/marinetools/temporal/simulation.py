...
def simulation(
    ...
    """Simulates time series with the probability model given in param and the temporal dependency given in param["TD"]
    ...
    # df_dt = read.rjson(param["TD"]["file_name"], "td")
    param = check_simulation_parameters(param)
            ...
            if param["TS"]["events"]:
                ...
                for season in param["TS"]["season"].keys():
                    ...
                    if any(durs_by_seasons[season]["storms"] < 0):
                        ...
                        durs_by_seasons[season]["calms"] = durs_by_seasons[season][
                            "calms"
                        ][indexs]
                        logger.info(
                ...
                while ini < end:
                    ...
                    # -------------------------------------------------------------------
                    zsim = var_simulation(
                        param["TD"], int(dstorm / factor) + 1, "normal"
                    )
...
def check_simulation_parameters(param):
...
def class_seasons(date, type_: str = "WSSF"):
